...
function moveAI(){
	...
	if ($("td").not(".o,.x").length>=8){
		var move = firstMove()
	} else {
		var move = minMax($("table"),0,CPU_T);
	}
	...
	if (winner){
		$(".winBox").css("display","inline");$("span").html(winner.toUpperCase());$("span").css("color",winner==="o"?"green":"red")
	} else if (winner == null){
		$("winBox").css("display","inline");$(".winBox > p").html("Null !!")
	};
	toggleTurn();
}
...
function firstMove(){
	...
	var z = {
		col : move[r].getAttribute("data-col"),
